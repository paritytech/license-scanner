...
const scanCrates = async function (
  rust: ScanOptionsRust,
  options: Omit<ScanOptions, "rust">,
) {
  const {
    root,
    saveResult,
  ...
  const project = await new Promise<RustCrateScannerOutput>(function (
    resolve,
    reject,
  ) {
    execute(
      rust.cargoExecPath,
      ["run", "--release", root, String(rust.shouldCheckForCargoLock)],
  ...
  if (project.license !== null && project.license !== undefined) {
    ...
    saveResult(initialRoot, transformItemKey("Cargo.toml"), {
  ...
  for (const { source, ...subCrate } of project.crates ?? []) {
    ...
    const cratePath = await (function () {
      switch (source.tag) {
        case "git": {
          return getOrDownloadRepository(
            dirs.repositories,
            new RepositoryCrate(subCrate, source),
            logger,
          )
        }
        case "crates.io": {
          return getOrDownloadCrate(
            dirs.crates,
            new CratesIoCrate(subCrate, source),
            logger,
          )
        }
        default: {
    ...
    const crateScanRoot = await new Promise<string>(function (resolve, reject) {
      /*
      ...
      */
      execute(rust.cargoExecPath, ["metadata", "--format-version=1"], {
        cwd: cratePath,
      })
        .then(function (stdout) {
          if (stdout) {
            ...
            for (const pkg of cargoMeta.packages) {
              if (
                pkg.version === subCrate.version &&
                pkg.name === subCrate.name
              ) {
            ...
            reject(
              `Path for crate ${subCrate.name} was not found in ${cratePath}`,
            )
          } else {
  ...
  toNextFile: for await (const file of walkFiles(root)) {
    ...
    for (const rule of detectionOverrides ?? []) {
      if (rule instanceof DetectionOverrideById) {
        ...
        logger.info(
          `Found STARTS_WITH override "${rule.value}" for file ${file.path}`,
        )
      }

      if (rule.result === null) {
        logger.info(
          `Skipping setting a result for ${file.path} because it was overridden with a null result`,
        )
        continue toNextFile
      }
      ...
      if (rule.contents !== null) {
        ...
        if (rule.contents != fileContents) {
          throw new Error(
            `Rule ${rule.value}'s provided file contents do not matching the contents of ${file.path}'`,
          )
        }
      }
    ...
    scanQueue.add(async function () {
      const result = await matchLicense(file.path)
      if (result === undefined) {
        return
      }
      saveResult(initialRoot, key, {
        ...result,
