...
const normalizeLicense = function (
  text: string,
...
export const loadLicensesNormalized = async function (
  ...
  const loadedLicenses: LicenseInput[] = Object.entries(await loadFiles(root))
    .map(function ([id, text]) {
      return [{ id, text: [text], match: "full" as const }]
    })
    .flat()
    .concat()
  ...
  return loadedLicenses
    ...
    .map(function ({ text: rawTexts, id: originalId, ...partialLicense }) {
      const id = options?.aliases.get(originalId) ?? originalId
      return rawTexts.map(function (rawText) {
...
const spdxLicenseIdentifierTailRegExp = tailRegexpGenerator(
  spdxLicenseIdentifierCaption,
).concat(new RegExp(`${spdxLicenseIdentifierCaption}[^a-zA-Z0-9]+$`))

const spdxLicenseIdentifierPrefix = `${spdxLicenseIdentifierCaption}[^a-zA-Z0-9]+`
const spdxLicenseIdentifierMatcher = new RegExp(
  `${spdxLicenseIdentifierPrefix}[^a-zA-Z0-9]+([^\n]+)\n`,
)

const triggerAccumulationRegExp = copyrightTailRegExp.concat(
  spdxLicenseIdentifierTailRegExp,
)

export const getLicenseMatcher = function (
  licenses: License[],
  startLinesExcludes?: string[],
) {
  const bufSize = Math.max(
    4096,
    ...licenses.map(function ({ text, match }) {
      switch (match) {
        case "fragment": {
  ...
  return async function (file: string) {
    ...
    let pendingCallbacksIfChunkNotAccumulated: Array<() => void> = []
    ...
    readBytes: while (
      (bytesRead = (await readAsync(fd, buf, 0, bufSize, offset)).bytesRead)
    ) {
      offset += bytesRead
        ...
        switch (license.match) {
          case "full": {
            ...
            const length = Math.min(
              prevStart + normalizedChunk.length,
              license.text.length,
            )
            if (
              license.text
                .substring(prevStart, length)
                .startsWith(normalizedChunk.substring(0, length - prevStart))
            ) {
              if (length === license.text.length) {
                return { license: license.id }
              } else {
                pendingCallbacksIfChunkNotAccumulated.push(function () {
          ...
          case "fragment": {
                ...
                if (
                  license.text.startsWith(
                    start.substring(
                      0,
                      Math.min(
                        license.text.length - start.length,
                        start.length,
                      ),
                    ),
                  )
            ...
            if (startOffset !== -1) {
              while (true) {
                const slice = normalizedChunk.substr(
                  startOffset,
                  license.text.length,
                )
            ...
            if (matchesFound) {
              ...
              if (!shouldAccumulateChunk) {
                ...
                for (let i = 0; i < license.needleStart.length; ++i) {
                  if (
                    normalizedChunk.endsWith(
                      license.needleStart.substring(0, i),
                    )
                  ) {
    ...
    if (isCopyrighted) {
      return {
        description:
          'Flagged because "copyright" was found somewhere within this file, but no licenses were detected',
      }
    }
  }
}
