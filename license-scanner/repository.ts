...
const fetchRepository = async function (
  tmpDir: string,
  ...
  if (!(await existsAsync(tarball))) {
    await downloadMutex.runExclusive(function () {
      logger.info(
        `Downloading repository ${crate.source.repository} (ref: ${crate.source.ref.value})`,
      )
      return download(getRepositoryDownloadUrl(crate), tarball)
    })
  }

  const extractionDir = await new Promise<{ path: string; remove: () => void }>(
    function (resolve, reject) {
      tmp.dir({ tmpdir: tmpDir }, function (err, path, remove) {
  ...
  await new Promise<void>(function (resolve, reject) {
    tar.x({ f: tarball, cwd: extractionDir.path }, undefined, function (err) {
      if (err === undefined) {
        resolve()
      } else {
        reject(err)
...
export const getOrDownloadRepository = async function (
  ...
  const targetRefDir = await ensureDir(
    joinPath(repositoriesDir, crate.source.ref.tag),
  )

  const repositoryPath = joinPath(
    targetRefDir,
    getVersionedRepositoryName(crate),
  )
