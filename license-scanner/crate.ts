...
export const getVersionedCrateName = function ({
  name,
  version,
}: Pick<Crate, "name" | "version">) {
  return `${name}-${version}`
}

// e.g. https://crates.io/api/v1/crates/addr2line/0.16.0/download
const getCrateDownloadURL = function ({
  name,
  version,
}: Pick<Crate, "name" | "version">) {
...
const fetchCrate = async function (
  crate: CratesIoCrate,
  destination: string,
  logger: Logger,
) {
  ...
  await new Promise<void>(function (resolve, reject) {
    tar.x({ f: tarball, cwd: dirname(destination) }, undefined, function (err) {
      if (err === undefined) {
        resolve()
      } else {
        reject(err)
...
export const getOrDownloadCrate = async function (
  cratesDir: string,
  crate: CratesIoCrate,
  logger: Logger,
) {
